...
Native macOS menu bar + dashboard app focused on lid-closed “keep agents running while the display sleeps,” with an **Awake** lane (IOPower idle-sleep assertions, display may sleep) and an optional **Connectivity** lane (jittered gateway TCP + HTTPS `HEAD` fallbacks for hotspot idle drops). Frozen **v1** scope and acceptance shorthand are in [`spec/molly-v1-frozen-spec.md`](spec/molly-v1-frozen-spec.md).
